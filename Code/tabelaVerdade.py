...
#pygame.display.set_caption("Tabela Interativa")
...
dicas = [
    "Dica da Fase 1: Lembre-se do OR",
    "Dica da Fase 2: Lembre-se do AND",
    "Dica da Fase 3: Lembre começa da direita para esquerda",
    "Dica da Fase 4: Lembre-se do se somente se"
]
...
def show_game_over_screen(window,font, screen,user_name):
    ...
    classificacao = 0
    classification(classificacao)
    with open('./classificacoes/classificacao.txt','a') as arquivo:
        arquivo.write("0"+user_name+'\n')
...
def show_congratulations_screen(window,font,screen,user_name):
    window.fill(green)
    congrats_text = font.render("Parabéns! Você concluiu o modulo da tabela verdade!", True, white)
    text_rect = congrats_text.get_rect(center=(400, 300))
    window.blit(congrats_text, text_rect)
    congrats_text2 = font.render("Agora vamos para modulo da equivalência lógica!", True, white)
...
def main(screen,user_name):
    ...
    screen_height = 600
    window = pygame.display.set_mode((screen_width, screen_height))
    pygame.display.set_caption("Logic T.E.I - Tabela Verdade")
    # Cores
    white = (255, 255, 255)
    black = (0, 0, 0)
    green = (0, 255, 0)
    ...
    while True:
        ...
        # Exibe a pontuação
        title_text = font.render("Tabela verdade", True, black)
        title_rect = title_text.get_rect(center=(screen_width // 2, table_y - 90))
        window.blit(title_text, title_rect)
        instruction_text = font.render("Clique com mouse em cada caixinha na última coluna", True, black)
        instruction_rect = instruction_text.get_rect(center=(screen_width // 2, table_y - 60))
        window.blit(instruction_text, instruction_rect)
        instruction_text2 = font.render("Preenchendo com as teclas T(True) ou F(False) e ao final valide", True, black)
        instruction_rect2 = instruction_text2.get_rect(center=(screen_width // 2, table_y - 30))
